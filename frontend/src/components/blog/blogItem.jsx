    ...
    return (
        <div className="dark:bg-gray-800 dark:text-gray-100">
            <div className="container max-w-4xl px-10 py-6 mx-auto rounded-lg shadow-sm dark:bg-gray-900">
                ...
                <div className="mt-3">
                    <Link rel="noopener noreferrer" to={`/blog/${blog.blogid}`} className="text-2xl font-bold hover:underline">{blog.title}</Link>
                    <p className="mt-2 line-clamp-3  ">{blog.content}</p>
                </div>

                <div className="flex items-center justify-between mt-4">
                    <Link rel="noopener noreferrer" to={`/blog/${blog.blogid}`} className="hover:underline text-black">Read more</Link>
                    <div>
                        <Link rel="noopener noreferrer" to={`/${blog.username}`} className="flex items-center">
                            ...
                            <span className="hover:underline text-teal-500 font-bold text-xl">{blog.username}</span>
                        </Link>
                    </div>
                </div>
